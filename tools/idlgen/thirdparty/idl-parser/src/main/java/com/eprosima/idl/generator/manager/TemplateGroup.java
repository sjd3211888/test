...
// 2024-06 ~ 2025-05: idlgen optimization: production efficiency improvement, etc.
// ------------------------------------------------------------------
...
public class TemplateGroup
{
    ...
    public void setAttribute(String attribute, TemplateGroup tg) {
        if (tg != null) {
            boolean isDebugEnabled = (Log.m_level <= Log.DEBUG);
            for (Entry<String, StringTemplate> entry : m_templates.entrySet()) {
                String key = entry.getKey();
                StringTemplate value = entry.getValue();
                StringTemplate template = tg.getTemplate(key);

                if (template != null) {
                    if (isDebugEnabled) {
                        Log.printDebug("setting attribute (TemplateGroup) to template group " + key + " from " +
                                       template.getName() + " to " + value.getName());
                    }
                    value.setAttribute(attribute, template.toString());
                }
            }
        }
    }
