...
@init{
    ...
    // Store old namespace.
    String name = null, old_scope = ctx.getScope(), old_namespace = ctx.getIdlNamespace();
    Token tk = null;
}
    :   ( KW_MODULE )
    {
    ...
    RIGHT_BRACE
    {
        ...
        ctx.setIdlNamespace(old_namespace);
...
@init {
        ...
        EQUAL const_exp { constValue=$const_exp.literalStr; }
    {
        if(typecode != null)
        {
            constDecl = new ConstDeclaration(ctx.getScopeFile(), ctx.isInScopedFile(), ctx.getExtraNamespace() + (ctx.getScope().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getScope(), constName, typecode, constValue, tk);
            ctx.addConstDecl(constDecl);
...
enum_member_exp returns [String literalStr = null]
@init{
    String aux = null;
}
        ...
        if(ttg!=null)
        {
            ...
            for(int count = 0; count < ttg.first().size(); ++count)
            {
                ...
                if (fw_name != null)
                {
                    ...
                    {
                        fw_name = scop + "::" + fw_name;
                    }
                }
                TypeDeclaration typedeclaration = (fw_name == null) ? new TypeDeclaration(ctx.getScopeFile(), ctx.isInScopedFile(), scop, name, ttg.first().get(count), tk) : ctx.getTypeDeclaration(fw_name);
                ...
                if (fw_name == null)
                {
                    ...
                    if(typedeclaration.getTypeCode().isUseBBMode()) {
                        ctx.setLastStructBBMode(true);
                    }
                }
...
@init {
    ...
    :   type_spec[annotation] declarators
    {
       if($type_spec.typecode != null)
       {
           ...
           {
               typedefTypecode = new AliasTypeCode(ctx.getExtraNamespace() + (ctx.getScope().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getScope(), $declarators.ret.get(count).first().first());
               ...
               if($declarators.ret.get(count).second() != null)
               {
                   ...
                   if (ctx.getMaxBufferSize() != 0 && ((ArrayTypeCode)$declarators.ret.get(count).second()).isIsInnerPlain() && ctx.getMaxBufferSize() <= Integer.parseInt(((ArrayTypeCode)$declarators.ret.get(count).second()).getEstimateSize()))
                    {
                        ((ArrayTypeCode)$declarators.ret.get(count).second()).setUseBBMode(true);
                    }
               }
               else
               {
...
@init{
    ...
    :   KW_STRUCT
        ...
        LEFT_BRACE member_list[structTP] RIGHT_BRACE
        {
            ...
            if (ctx.isZeroCopy() && ctx.checkTopStructName(name))
            {
                structTP.setTopStruct(true);
            }
            if (ctx.getMaxBufferSize() != 0 && (structTP.isIsPlain() || structTP.isTop()) && ctx.getMaxBufferSize() <= Integer.parseInt(structTP.getEstimateSize()))
            {
                structTP.setUseBBMode(true);
            }
        }
    |
        KW_STRUCT
...
@init {
    ...
    :   KW_UNION
        ...
        KW_SWITCH LEFT_BRACKET switch_type_spec { dist_type=$switch_type_spec.typecode; } RIGHT_BRACKET
        {
            ...
            {
                unionTP = new UnionTypeCode(ctx.getExtraNamespace() + (ctx.getScope().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getScope(), name, dist_type, ctx.getExtraNamespace() + (ctx.getIdlNamespace().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getIdlNamespace());
            }
            unionTP.setDefined();
    ...
    |
        ...
        identifier
        {
            name=$identifier.id;
            // TODO Check supported types for discriminator: long, enumeration, etc...
            unionTP = new UnionTypeCode(ctx.getExtraNamespace() + (ctx.getScope().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getScope(), name, ctx.getIdlNamespace());
...
@init{
    ...
    :   KW_ENUM
        identifier
        {
            ...
            name = ctx.removeEscapeCharacter($identifier.id);
            enumTP = new EnumTypeCode(ctx.getExtraNamespace() + (ctx.getScope().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getScope(), name, ctx.getExtraNamespace() + (ctx.getIdlNamespace().isEmpty() || ctx.getExtraNamespace().isEmpty() ? "" : "::") + ctx.getIdlNamespace());
        }
...
enum_member_exp_opt returns [String literalStr = null]
    :   enum_member_exp { $literalStr = $enum_member_exp.literalStr; }
    ;
enumerator [EnumTypeCode enumTP]
@init{
    String name = null, enumValue = null;
    ArrayList<Annotation> annotations = new ArrayList<Annotation>();
}
    :   (annotation_appl { annotations.add($annotation_appl.annotation); })* identifier
        (EQUAL enum_member_exp { enumValue = $enum_member_exp.literalStr; })?
        {
            String error = ctx.checkIdentifier(Definition.Kind.TYPE_DECLARATION, ctx.getScope(), $identifier.id);
            if (error != null)
            {
            ...
            name = ctx.removeEscapeCharacter($identifier.id);
            EnumMember new_field = new EnumMember(name, enumValue);
            for (Annotation ann : annotations)
            {
...
@init {
    TypeCode type = null;
    String maxsize = null, valuesize = null;
    Definition def = null;
}
    :   ( (KW_SEQUENCE)
    ...
    |   (KW_SEQUENCE)
        ...
        {
            valuesize = maxsize;
            if(maxsize != null && !Pattern.matches("\\d+", maxsize))
            {
                ConstDeclaration tmp_const = ctx.getConstDecl(maxsize, ctx.getScope());
                if(tmp_const != null)
                {
                    valuesize = tmp_const.getValue();
                    maxsize = tmp_const.getScopedname();
                } else {
                EnumMember tmp_enum = ctx.getEnumDecl(maxsize, ctx.getScope());
                if(tmp_enum != null)
                {
                        valuesize = tmp_enum.getValue();
                        maxsize = tmp_enum.getValue();
                }
                }
            }
            if(type != null)
            {
                $typecode = new SequenceTypeCode(maxsize, valuesize);
            ...
            if (ctx.getMaxBufferSize() != 0 &&
                ((((Sequence_typeContext)_localctx).typecode.isSetLength() &&
                   (ctx.getMaxBufferSize() <= Integer.parseInt(((Sequence_typeContext)_localctx).typecode.getEstimateSize()))
                    ) ||
                    (
                        ((Sequence_typeContext)_localctx).typecode.isUnbound() &&
                        ctx.isUseStaticCapacity() &&
                        ctx.getMaxBufferSize() <= ctx.getStaticCapacity()
                    )
                )
            )
            {
                ((Sequence_typeContext)_localctx).typecode.setUseBBMode(true);
            }
        }
    ;
    ...
    |   KW_SET
        LEFT_ANG_BRACKET simple_type_spec[null] { type=$simple_type_spec.typecode; def=$simple_type_spec.def; } RIGHT_ANG_BRACKET )
        {
           ...
           if(maxsize != null && !Pattern.matches("\\d+", maxsize))
           {
            ...
            $typecode = new SetTypeCode(maxsize, valuesize);
            if (type != null)
            {
                $typecode.setContentTypeCode(type);
            }
    ...
    String maxsize = null, valuesize = null;
        ...
        RIGHT_ANG_BRACKET
        {
           ...
           if(maxsize != null && !Pattern.matches("\\d+", maxsize))
           {
            ...
            $typecode = new MapTypeCode(maxsize, valuesize);
    ...
    |   KW_STRING )
       {
        ...
        $typecode = new StringTypeCode(Kind.KIND_STRING, maxsize, valuesize, ctx.isNotUseVbsFramework());
        if (ctx.getMaxBufferSize() != 0 && ((String_typeContext)_localctx).typecode.isSetLength() && ctx.getMaxBufferSize() <= Integer.parseInt(((String_typeContext)_localctx).typecode.getEstimateSize()))
            {
                ((String_typeContext)_localctx).typecode.setUseBBMode(true);
            }
        }
    ;
    ...
    |   KW_WSTRING )
       {
        ...
        $typecode = new StringTypeCode(Kind.KIND_WSTRING, maxsize, valuesize, ctx.isNotUseVbsFramework());
    }
    ;
...
@init
{
    ...
    :   ID
        (
            fixed_array_size
            {
               maxsize = $fixed_array_size.literalStr;
               valuesize = maxsize;
                if(maxsize != null && !Pattern.matches("\\d+", maxsize))
                {
                    int lastIndex = maxsize.lastIndexOf("::");
                        ConstDeclaration tmp_const = null;
                    tmp_const = ctx.getConstDecl(maxsize, ctx.getScope());

                    if(tmp_const != null)
                    {
                        valuesize = tmp_const.getScopedname();
                        typecode.addConstMap(valuesize, ArrayTypeCode.evaluateMathExpression(tmp_const.getValue()));
                    } else {
                        EnumMember tmp_enum = ctx.getEnumDecl(maxsize, ctx.getScope());
                        if(tmp_enum != null)
                        {
                            valuesize = ctx.getEnumDeclScopname(maxsize, ctx.getScope());
                            valuesize = String.valueOf(ArrayTypeCode.evaluateMathExpression(tmp_enum.getValue()));
                        }
                    }
                }
                typecode.addDimension(valuesize);
            }
        )+
        {
...
readonly_attr_declarator returns [List<Pair<Pair<String, Token>, ContainerTypeCode>> retList = new ArrayList<>()]
    :   simple_declarator raises_expr
    |   simple_declarator {$retList.add($simple_declarator.ret);} ( COMA simple_declarator {$retList.add($simple_declarator.ret);})*
    ;
...
@init
{
    ...
    | identifiers
    {
        List<String> retList = $identifiers.retList;

        if (retList.size() == 2 || retList.size() == 3) {
            String sufix_value = retList.get(retList.size() - 1);
            if (annotation != null && !annotation.addValue("vbsconfig_value", retList.get(0), sufix_value)) {
                throw new ParseException(tk, "is not an attribute of annotation " + annotation.getName());
            }
        } else {
            throw new ParseException(null, "Expected 2 or 3 attributes, but got " + retList.size());
        }
    }
    )
    ;

identifiers returns [List<String> retList = new ArrayList<>()]
    :   identifier {$retList.add($identifier.id);} ( COMA (identifier {$retList.add($identifier.id);}|const_exp {$retList.add($const_exp.literalStr);}))*
    ;
...
KW_VBSCONFIG:           'vbsconfig';

ID
    :   LETTER (LETTER|ID_DIGIT|DOUBLE_COLON)*
    ;
