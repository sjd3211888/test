...
#ifndef _EDDS_TOPIC_CONTENTFILTERINFO_HPP_
#define _EDDS_TOPIC_CONTENTFILTERINFO_HPP_
 ...
 */
inline bool cdr_serialized_size(std::size_t num_filters, uint16_t& cdr_size) {
    bool ret_val = true;
    cdr_size = 0;
...
inline void add_bitmap(evbs::ertps::rtps::CDRMessage_t& msg, uint32_t current_bitmap, uint32_t& bitmap_pos) {
    uint32_t old_len = msg.length;
    uint32_t old_pos = msg.pos;
    msg.pos = bitmap_pos;
    msg.length = bitmap_pos;
    evbs::ertps::rtps::CDRMessage::addUInt32(&msg, current_bitmap);
    bitmap_pos = msg.pos;
    msg.pos = old_pos;
    msg.length = old_len;
}
...
void cdr_serialize(evbs::ertps::rtps::SerializedPayload_t& payload, std::size_t num_filters, FilterEvaluate evaluate) {
    ...
    if ((0 < num_filters) && cdr_serialized_size(num_filters, cdr_size)) {
        ...
        // PID + PLENGTH
        CDRMessage::addUInt16(&msg, vbsutil::xmlparser::PID_CONTENT_FILTER_INFO);
        ...
        for (std::size_t i = 0; i < num_filters; ++i) {
            ...
            if (msg.pos < msg.max_size && evaluate(i, &msg.buffer[msg.pos])) {
...
#endif  // _EDDS_TOPIC_CONTENTFILTERINFO_HPP_
