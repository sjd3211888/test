...
#ifndef INCLUDE_EDDS_DDS_CORE_LOANABLESEQUENCE_HPP_
#define INCLUDE_EDDS_DDS_CORE_LOANABLESEQUENCE_HPP_
 ...
 * EDDS, a '<tt>using FooSeq = LoanableSequence<Foo></tt>' is generated. The sequence offers a
 * subset of the methods defined by the standard OMG IDL to C++ mapping for sequences. We refer to
 * an IDL '<tt>sequence<Foo></tt>' as \c FooSeq.
 *
 * The state of a sequence is described by the properties 'maximum', 'length' and 'has_ownership'.
 * @li The 'maximum' represents the size of the underlying buffer; this is the maximum number of
 * elements it can possibly hold. It is returned by the maximum() operation.
 * @li The 'length' represents the actual number of elements it currently holds. It is returned by
 * the length() operation.
 * @li The 'has_ownership' flag represents whether the sequence owns the underlying buffer. It is
 * returned by the has_ownership() operation. If the sequence does not own the underlying buffer,
 * the underlying buffer is loaned from somewhere else. This flag influences the lifecycle of the
 * sequence and what operations are allowed on it. The general guidelines are provided below and
 * more details are described in detail as pre-conditions and post-conditions of each of the
 * sequence's operations:
 *
 *     @li If has_ownership == true, the sequence has ownership on the buffer. It is then
 * responsible for destroying the buffer when the sequence is destroyed.
 *     @li If has_ownership == false, the sequence does not have ownership on the buffer. This
 * implies that the sequence is loaning the buffer. The sequence should not be destroyed until the
 * loan is returned.
...
class LoanableSequence : public evbs::edds::dds::LoanableTypedCollection<T, _NonConstEnabler> {
 public:
     ...
     * When the input parameter is less than or equal to 0, the behavior is equivalent to the
     * default constructor. Otherwise, the post-conditions below will apply.
     *
     ...
     */
    LoanableSequence(const LoanableSequence& other) { *this = other; }
    ...
    LoanableSequence& operator=(const LoanableSequence& other) {
        ...
        evbs::edds::dds::LoanableCollection::length(other.length());
...
#define EDDS_SEQUENCE(FooSeq, Foo) using FooSeq = vbs::LoanableSequence<Foo>
#define EDDS_CONST_SEQUENCE(FooSeq, Foo) using FooSeq = vbs::LoanableSequence<Foo, std::false_type>

#endif  // INCLUDE_EDDS_DDS_CORE_LOANABLESEQUENCE_HPP_
